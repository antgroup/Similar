...
# parser.add_argument(
#     "--device", type=int, default=1, help="CUDA device index to use for computation"
# )
...
if torch.cuda.device_count() > 1:
    ...
    device_map = infer_auto_device_map(rm, max_memory=get_balanced_memory(rm))
    rm = dispatch_model(rm, device_map)
...
for example in tqdm(ds, desc="Processing dataset"):
    ...
    message_1 = message[0]
    list_temp = [message_1]
    # print("type(message) = ", type(message[0]['content'][0]['text']))
    if args.model_path.endswith("FsfairX-LLaMA3-RM-v0.1"):
        ...
        conv_formatted = rm_tokenizer.apply_chat_template(list_temp, tokenize=False)
...
save_path = "/mnt/prev_nas/virtual_agent/MBC/RLHF-Reward-Modeling/checkpoints/regression_internvl2_5_embedding"
