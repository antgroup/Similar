...
import datasets
import numpy as np
from transformers import MllamaForConditionalGeneration, AutoTokenizer, AutoModel, AutoProcessor
...
parser.add_argument(
    "--model_path",
    type=str,
    default="/mnt/prev_nas/virtual_agent/models/llm_research/Llama-3.2-11B-Vision-Instruct",  # https://huggingface.co/sfairXC/FsfairX-LLaMA3-RM-v0.1
    help="Path to the pre-trained model (HuggingFace path or local folder)",
)
parser.add_argument(
    "--dataset_path",
    type=str,
    default="/mnt/prev_nas/virtual_agent/MBC/Reward_Model_csv_zeta/Reward_Model_multi-objective_dataset.csv",  # https://huggingface.co/datasets/RLHFlow/ArmoRM-Multi-Objective-Data-v0.1
...
parser.add_argument(
    "--device", type=int, default=2, help="CUDA device index to use for computation"
)
...
rm = MllamaForConditionalGeneration.from_pretrained(
    args.model_path,
...
for example in tqdm(ds, desc="Processing dataset"):
    ...
    image = Image.open(requests.get(example["observation_url"], stream=True).raw)

    conv_formatted = processor.apply_chat_template(message, tokenize=False, add_generation_prompt=False)
    # conv_formatted = rm_tokenizer.apply_chat_template(message, tokenize=False, add_generation_prompt=False).replace(rm_tokenizer.bos_token, "")

    # Tokenize the formatted conversation and move tensors to the specified device  对格式化的对话进行tokenize，并将张量移动到指定设备
    conv_tokenized = processor(image, conv_formatted, return_tensors="pt").to(device)
    ...
    with torch.no_grad():
        ...
        # embeddings.append(output.hidden_states[0][-1].cpu())
        # print("\noutput = ", output)
        # print("\n")
        embeddings.append(output.logits.cpu())
...
save_path = "/mnt/prev_nas/virtual_agent/MBC/RLHF-Reward-Modeling/checkpoints/regression_llama3.2_embedding"
