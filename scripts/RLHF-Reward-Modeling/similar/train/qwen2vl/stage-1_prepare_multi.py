...
parser.add_argument(
    "--shard_idx", type=int, default=4, help="Index of the current shard"
)
# parser.add_argument(
...
rm = DDP(rm, device_ids=[local_rank], output_device=local_rank, find_used_parameters=True)
...
for example in tqdm(ds, desc="Processing dataset"):
    # Format the conversation messages using the tokenizer's chat template without tokenization  使用tokenizer的chat template格式化对话消息，无需tokenization
