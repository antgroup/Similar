...
def eval_grm_bench(df_examples, acc_column="correct"):
    ...
    row = {
        "type": 'avg',
        "num": len(df_examples),
        "accuracy": df_examples[acc_column].values.mean(),
    }
    # print("\nrow = ", row)
    df_acc = pd.concat([df_acc, pd.DataFrame(row, index=[0])], ignore_index=True)
...
parser.add_argument("--model_path", type=str, default="/mnt/prev_nas/virtual_agent/MBC/ArmoRM/ArmoRM-Llama3-8B-v0.1")
parser.add_argument(
...
args.embedding_path = f"/mnt/prev_nas/virtual_agent/MBC/RLHF-Reward-Modeling/checkpoints/gating_network_armo_embedding_train_prompt_1.7-train.safetensors"
args.regression_layer_path = "/mnt/prev_nas/virtual_agent/MBC/RLHF-Reward-Modeling/checkpoints/regression_armo_checkpoint.pt"
...
embedding_path = f"/mnt/prev_nas/virtual_agent/MBC/RLHF-Reward-Modeling/checkpoints/gating_network_armo_embedding_train_prompt_1.7-train.safetensors"
...
gating_network.load_state_dict(torch.load("/mnt/prev_nas/virtual_agent/MBC/RLHF-Reward-Modeling/checkpoints/gating_network_armo.pt", map_location = device))
...
# save_path = "/mnt/prev_nas/virtual_agent/MBC/RLHF-Reward-Modeling/checkpoints/gating_network_llama3.pt"
...
reward_bench_embeddings, reward_bench_prompt_embeddings = load_embeddings(
    "/mnt/prev_nas/virtual_agent/MBC/RLHF-Reward-Modeling/checkpoints/gating_network_armo_embedding_test_prompt-train.safetensors", device=device
)
...
print(type(correct_rb.cpu().numpy()))
# print("correct_rb:\n")
# print(correct_rb.cpu().numpy())
correct_list = correct_rb.cpu().numpy().tolist()
print("len(correct_list) = ", len(correct_list))
print("\ntotl accuracy:")
print(correct_list.count(1)/len(correct_list))
print("\n")


# reward_bench_ds = datasets.load_dataset("csv", data_files="/mnt/prev_nas/virtual_agent/MBC/Reward_Model_csv_zeta/Reward_Model_preference_dataset_test.csv", split="train")
# reward_bench_ds = pd.read_csv("/mnt/prev_nas/virtual_agent/MBC/Reward_Model_csv_zeta/Reward_Model_preference_dataset_test.csv")
reward_bench_ds = pd.read_csv("/mnt/prev_nas/virtual_agent/MBC/Reward_Model_csv_zeta/Reward_Model_preference_dataset_test_prompt.csv")
print("\n")
print("type(reward_bench_ds) = ", type(reward_bench_ds))
# df_examples = pd.DataFrame(
#     {"dataset": reward_bench_ds, "correct": correct_rb.cpu().numpy()}
# )
reward_bench_ds['correct'] = [score for score in correct_list]

acc_per_type = eval_grm_bench(reward_bench_ds)
print("\nGRMBench Scores:")
print(acc_per_type)