...
srm_ok_csv_file = "SRM_test_ok.csv" # 筛掉不合理数据的srm数据

srm_original_filter_csv_file = "SRM_test_original_filter.csv" # 已经去重的srm数据
...
for index2, row2 in df_filter.iterrows():
    if row2['type'] == 'traj':
        continue
    id = row2['compare_id'] + '_' + str(row2['step_idx']) + '_' + row2['type']
    action_map[id] = []
        ...
        if row1['type'] == 'traj':
            writer.writerow(
                {'compare_id': row1['compare_id'], 'step_idx': row1['step_idx'],
                 ...
                 'result': row1['result'], 'rejmected_modified': 'None'})
            continue
        for index2, row2 in df_filter.iterrows():
                ...
                if (row1['compare_id'] == row2['compare_id']) and (row1['step_idx'] == row2['step_idx']) and (row1['type'] == row2['type']): # 就是该数据
                    writer.writerow(
                        {'compare_id': row1['compare_id'], 'step_idx': row1['step_idx'],
                         ...
                         'reason_steps': row1['reason_steps'], 'image_url': row1['image_url'],
                         'chosen': row1['chosen'], 'rejected': row1['rejected'],
                         'result': row1['result'], 'rejected_modified': 'None'})
                    continue
                else: # 是其它的重复数据
                    id = row2['compare_id'] + '_' + str(row2['step_idx']) + '_' + row2['type']
                    rejected_action_set = '{' + ',\n'.join(map(str, action_map[id])) + '}'
                    print("rejected_action_set = \n", rejected_action_set)
                    new_action = generate_action(row2, rejected_action_set)
