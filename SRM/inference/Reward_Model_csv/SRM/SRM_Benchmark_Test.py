...
5. [Coherence]. 
    5.1 Meaning: It indicates the compactness and coherence between the current step and the previous step. 
...
6. [Total].
    Meaning: It integrates decision-making based on the 5 dimensions mentioned earlier.
7. [Trajectory].
    Meaning: It indicates the evaluation of the entire trajectory of the agent completing the task. For example, if trajectory A can complete the task but trajectory B cannot, it is considered that trajectory A is better than trajectory B. If both trajectory a and trajectory b can complete the task, and trajectory a has fewer steps, then trajectory a is considered superior to trajectory b.
</EVALUATION DIMENSION>
\n
...
def eval_grm_bench(df_examples):
    types = [
        "IP",
        "E",
        "TC",
...
df_result = pd.DataFrame(columns=['compare_id', 'step_idx', 'instruction', 'type', 'reason_steps', 'image_url', 'chosen', 'rejected', 'answer', 'result', 'correct'])
...
for index, row in benchmark_dataest.iterrows():
    ...
    if (random_flag == 1):
        ...
        user_message = user_message.replace("<ACTION_1>", action2)
        user_message = user_message.replace("<ACTION_2>", action1)
    ...
    while True:
        ...
        if (response != False) and ('choose' in response):
            ...
            # print(response)
            choose = int(response['choose'])
            reason = response["reason"]
            # results_list.append({"compare_id": compare_id, "choose": choose, "reason": reason})
            if choose == random_flag:
                correct = 1
            else:
                correct = 0
            df_result.loc[len(df_result)] = [compare_id, step_idx, instruction, type, trajectory, observation, random_flag, choose, correct]
            break


acc_per_type = eval_grm_bench(df_result)
print("\nSRMBench Scores:")
print(acc_per_type)