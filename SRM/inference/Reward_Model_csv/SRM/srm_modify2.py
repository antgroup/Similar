...
final_csv_file = "SRM_test_ok_modify2.csv"
...
srm_ok_csv_file = "SRM_test_ok.csv" # 筛掉人工标注时不合理数据的srm数据

srm_original_filter_csv_file = "SRM_test_ok_modify_filter.csv" # 已经去重的srm数据+修改的数据

df_ok = pd.read_csv(srm_ok_csv_file)

df_filter = pd.read_csv(srm_original_filter_csv_file)
                ...
                if (row1['compare_id'] == row2['compare_id']) and (row1['step_idx'] == row2['step_idx']) and (row1['type'] == row2['type']): # 就是该数据
                    writer.writerow(
                        {'compare_id': row1['compare_id'], 'step_idx': row1['step_idx'],
                         ...
                         'result': row1['result'], 'rejected_modified': row2['rejected_modified']})
                    flag = 1
                    break
                else: # 是其它的重复数据
                        ...
                        if (row1['type'] == 'traj'):
                            if not (row3['type'] == 'traj'):
                                ...
                                if not (row1['instruction'] == row3['instruction']):
                                    if (row1['rejected'] != row3['rejected']) and (row3['rejected'] not in action_map[id]):
                                        writer.writerow(
                                            {'compare_id': row1['compare_id'], 'step_idx': row1['step_idx'],
                                             'instruction': row1['instruction'], 'type': row1['type'],
                                             'reason_steps': row1['reason_steps'], 'image_url': row1['image_url'],
                                             'chosen': row1['chosen'], 'rejected': row1['rejected'],
                                             'result': row1['result'], 'rejected_modified': row3['rejected']})
                                        flag = 1
                                        break
                                    elif (row1['rejected'] != row3['chosen']) and (row1['chosen'] != row3['chosen']) and (row3['rejected'] not in action_map[id]):
                                        writer.writerow(
                                            {'compare_id': row1['compare_id'], 'step_idx': row1['step_idx'],
                                             'instruction': row1['instruction'], 'type': row1['type'],
                                             'reason_steps': row1['reason_steps'], 'image_url': row1['image_url'],
                                             'chosen': row1['chosen'], 'rejected': row1['rejected'],
                                             'result': row1['result'], 'rejected_modified': row3['chosen']})
                                        flag = 1
                                        break
                        else:
                            if not (row1['instruction'] == row3['instruction']):
                                if (row1['rejected'] != row3['rejected']) and (row3['rejected'] not in action_map[id]):
                                    ...
                                    flag = 1
                                    break
                                elif (row1['rejected'] != row3['rejected_modified']) and (row1['chosen'] != row3['rejected_modified']) and (
                                        row3['rejected_modified'] not in action_map[id]) and (row3['rejected_modified'] != 'nan'):
                                    writer.writerow(
                                        {'compare_id': row1['compare_id'], 'step_idx': row1['step_idx'],
                                         'instruction': row1['instruction'], 'type': row1['type'],
                                         'reason_steps': row1['reason_steps'], 'image_url': row1['image_url'],
                                         'chosen': row1['chosen'], 'rejected': row1['rejected'],
                                         'result': row1['result'], 'rejected_modified': row3['rejected_modified']})
                                    flag = 1
                                    break
        ...
        if flag == 0:
            ...
            writer.writerow(
                {'compare_id': row1['compare_id'], 'step_idx': row1['step_idx'],
                 ...
                 'reason_steps': row1['reason_steps'], 'image_url': row1['image_url'],
                 'chosen': row1['chosen'], 'rejected': row1['rejected'],
                 'result': row1['result'], 'rejected_modified': new_action})