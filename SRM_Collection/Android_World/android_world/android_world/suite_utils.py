...
def _run_task(
    ...
    try:
        ...
        task_successful = task.is_successful(env) # 任务是否成功
        ...
        agent_successful = task_successful if interaction_results.done else 0.0 # 计算任务是否成功的指标
        print(
...
def _run_task_suite(
    ...
    for name, instances in suite.items():
        ...
        for i, instance in enumerate(instances):
            ...
            # 任务是否已经在被某个进程处理
            doing_file = "doing.json"
            doing = []

            if not os.path.exists(doing_file):
                with open(doing_file, "w") as f:
                    json.dump(doing, f)
            else:
                with open(doing_file, "r") as f:
                    doing = json.load(f)

            print("doing = ", doing)

            if instance.name in doing:
                continue
            else:
                doing.append(instance.name)
                print("doing = ", doing)
                with open(doing_file, "w") as f:
                    json.dump(doing, f)
