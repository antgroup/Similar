...
def ask_chatgpt(model, msg, img, temp='0.2'):
    ...
    param["queryConditions"]["messages"][0]["content"][1]["image_url"] = {"url": f'data:image/jpeg;base64,{encode_image(img)}'}
...
class M3A(base_agent.EnvironmentInteractingAgent):
    ...
    def get_TR_C(self, goal, steps, action_last, action):
        print("\nget_TR_C\n")
        now_prompt = get_TR_C_prompt.replace("{Instruction}", goal)
        ...
        img = state.pixels.copy()


        while True:
            response = ask_chatgpt(model, msg, img, temperature_str)
    ...
    def screenshot(self, screenfile):
        ...
        screenshot = array_to_jpeg_bytes(screenshot)
        with open(screenfile, "wb") as _f:
            _f.write(screenshot)

    def trans_action(self, action, summary_origin):
        print("\ntrans_action\n")
        print("action = ", action)
        ...
        if 'index' in action:
            if 'text' in action:
                ...
                for (key, value) in action_dict.items():
                    if (key == 'index'):
                        continue
                    action_content += ' ' + str(value)

                return action_content

            action_content = str(action_type)

            element_idx = int(action_dict['index'])

            before_ui_elements = self.before_ui_elements
            for index, ui_element in enumerate(before_ui_elements):
                if index == element_idx:
                    ...
                    print("\n")
                    if ui_element.content_description:
                        action_content += ' ' + ui_element.content_description
                    elif ui_element.text:
                        action_content += ' ' + ui_element.text
                    else:
                        action_content = summary_origin
                    break
    ...
    def execute(self, goal: str, action: str, reason: str):
        step_data = {
            'raw_screenshot': None,
        ...
        self.before_ui_elements = before_ui_elements
        ...
        summary, is_safe = self.llm.predict_mm(
            summary_prompt,
            [
                before_screenshot,
                after_screenshot,
        ...
        step_data['summary_origin'] = summary
    ...
    def step(self, goal: str) -> base_agent.AgentInteractionResult:
        ...
        # 预测，输出结果
        action_output, is_safe = self.llm.predict_mm(
            action_prompt,
            [
                step_data['raw_screenshot'],
                before_screenshot,
...
Action: {"action_type": "status", "goal_status": "infeasible"}"""
        ...
        action = action[action.find("{") : action.find("}") + 1]

        # 确定当前步骤的Action和Reason
        # print("\n")
        print('Action: ' + action)
        print('Reason: ' + reason)
        print("\n")
